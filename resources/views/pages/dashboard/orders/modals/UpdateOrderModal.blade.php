<div class="modal fade" id="order-update-modal-{{$foodService->id}}" tabindex="-1" role="dialog" aria-labelledby="order-update-modal-label"
     aria-hidden="true">
    <form id="update-employee-form" role="form" method="POST" enctype="multipart/form-data"
          action="{{ route('order-update', ['id' => $foodService->id]) }} ">
        @method('PUT')
        @csrf
...
@if($errors->has('nameUpdate')         ||
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $("#order-update-modal-{{$foodService->id}}").modal("show");
        });
    </script>
@endif
